...
function onDeviceReady() {
    ...
    accura = cordova.plugins.CordovaOCRPlugin;
        ...
        if(results.isValid) {
            ...
            if(results.isMRZ) {
                for (let i = 0; i < mrzNames.length; i++) {
                    $("#tables").append(
                        "<thead class='mrz-head'><tr><td><button class='btn btn-round' style='width:100%;height:50px;background-color: #818589;color: white;font-size: 20px;' onclick='startMRZ(this.value)' value='" + mrzTypes[i] + "'>" + mrzNames[i] + "</button></td></tr></thead>"
                    )
                }
            }
            ...
            if (results.isBarcode) {
                ...
                $("#tables").append(
                    "<thead class='barcode-head'><tr><td><button class='btn btn-round' style='width:100%;height:50px;background-color: #818589;color: white;font-size: 20px;' onclick='startBarcode1()'>Barcode</button></td></tr></thead>"
                )
            }

            if (results.isBankcard) {
                $("#tables").append(
                        "<thead class='bank-head'><tr><td><button class='btn btn-round' style='width:100%;height:50px;background-color: #818589;color: white;font-size: 20px;' onclick='startBankcard()'>Bank Card</button></td></tr></thead>"
                )
            }
            ...
            if(results.isOCR) {
                if(results.countries.length > 0) {
                    ...
                    results.countries.forEach(function (country, i) {
                        ...
                        $("#tables").append(
                            "<thead class='ocr-head'><tr><td><button class='btn btn-danger btn-round' style='width:100%;height:50px;color: white;font-size: 20px;' onclick='getCardModal(this.id)' id='" + i + "'>" + country.name + "</button></td></tr></thead>"
                        )
                    })
                }
            }
...
function getCardModal(id) {
    ...
    countryCard.forEach(function(card, i) {
        $("#tables").append(
            "<thead class='ocr-card-head'><tr><td><button class='btn btn-danger btn-round' style='width:100%;height:50px;color: white;font-size: 20px;' onClick='openForCard(this.id)' id='" + i + "'>" + card.name + "</button></td></tr></thead>"
        )
    });
}
...
function startBarcode1() {
    ...
    for (let i = 0; i < barcodesNames.length; i++) {
        $("#tables").append(
            "<thead class='barcode-types'><tr><td><button class='btn btn-round' style='width:100%;height:50px;background-color: #818589;color: white;font-size: 20px;' onclick='startBarcode(this.value)' value='" + barcodesTypes[i] + "'>" + barcodesNames[i] + "</button></td></tr></thead>"
        )
    }
